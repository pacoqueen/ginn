...
## XXX
# Modificaciones by Queen (CWT):
# Para que las búsquedas sean "case insensitives" he considerado 3 opciones:
# 1.- Usar ILIKE (como LIKE pero sin diferenciar mayúsculas de minúsculas).
# 2.- Hacer los SELECT con WHERE UPPER(campo) = UPPER('cadena');
#       Ver funciones LOWER() y UPPER() o STRTOLOWER() y STRTOUPPER().
# 3.- Usar expresiones regulares (no sé si todos los motores de base de datos las soportan): operador ~[+|*] en PostgreSQL:
#       (~ indica que usa regexp y * es el "ignore case").
#       SELECT ... WHERE campo ~* 'valor';
# A mí me sigue pareciendo una PÉSIMA idea hacer todas las búsquedas así, pero el cliente manda.
# De momento uso el método ILIKE, ya que todas las búsquedas se basan en .contains que a su vez
# se implementa sobre el LIKE que he modificado aquí.
## XXX
...
class SQLExpression:
    ...
    def __repr__(self):
# XXX: TEMP: by queen. ----------8<----------
        return self.__sqlrepr__('postgres')
# XXX: TEMP: by queen. ---------->8----------
...
#ByQUEEN:        elif self.op == "LIKE":
        elif self.op == "LIKE" or self.op == "ILIKE":
...
#ByQUEEN:    return SQLOp("LIKE", expr, string)
    return SQLOp("ILIKE", expr, string)

def STARTSWITH(expr, string):
#ByQUEEN:    return SQLOp("LIKE", expr, _LikeQuoted(string) + '%')
    return SQLOp("ILIKE", expr, _LikeQuoted(string) + '%')
