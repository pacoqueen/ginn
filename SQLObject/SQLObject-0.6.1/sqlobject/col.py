...
# By Queen
import sys, os
if "framework" in os.path.abspath(os.path.curdir):  # Me está importando pclases y fixedpoint no está donde creo que está. La ruta debe
                                                    # ser relativa a pclases, no a sqlobject.
    ruta_fixedpoint = os.path.join("..", "formularios")
else:
    ruta_fixedpoint = os.path.join("..", "..", "..", "formularios")
    if not os.path.exists(ruta_fixedpoint):
        ruta_fixedpoint = os.path.join("..", "..", "formularios")
    if not os.path.exists(ruta_fixedpoint):
        ruta_fixedpoint = os.path.join("..", "formularios")
sys.path.append(ruta_fixedpoint)
...
# By Queen: Quiero darle preferencia a MX sobre el datetime nativo
#if datetime_available:
...
#        if not isinstance(value, (int, long, float, sqlbuilder.SQLExpression)):
        if not isinstance(value, (int, long, float, sqlbuilder.SQLExpression, FixedPoint)):
        # EOBQ
    ...
    class DateTimeValidator(validators.DateValidator):
        def fromPython(self, value, state):
            if isinstance(value, time.struct_time):
                valuemx = DateTime.DateTimeFrom(year = value.tm_year, 
                                                month = value.tm_mon, 
                                                day = value.tm_mday, 
                                                hour = value.tm_hour, 
                                                minute = value.tm_min, 
                                                second = value.tm_sec) 
                return valuemx.strftime(self.format)
            if value is None:
                return None
