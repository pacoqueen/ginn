...
###################################################################
## albaranes_de_entrada_cs21.py -- Albaranes de entrada de 
##   mercancía (compras). Adaptado a CRISVA Siglo XXI.
###################################################################
## 
###################################################################
...
#                           padre = self.wids['ventana'])
    ...
    def inicializar_ventana(self):
        ...
        for t in pclases.Tarifa.select(orderBy = "nombre"):
            if t.vigente:
                cols_tarifas.append((t.nombre + " c/IVA", 
                                     'gobject.TYPE_STRING', 
                                     False, 
                                     True, 
                                     False, 
                                     None))
                idstarifas.append(t.id)
        cols = list(cols[:-1]) + cols_tarifas + [cols[-1]]
        utils.preparar_listview(self.wids['tv_ldvs'], cols)
        for n in [10 + i for i in range(len(cols_tarifas))]:
