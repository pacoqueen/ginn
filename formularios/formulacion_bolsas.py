...
###################################################################
## formulacion_bolsas.py - Formulación HARCODED de la línea de 
...
class FormulacionBolsaCemento(Ventana):
    def __init__(self, objeto = None, usuario = None):
        ...
        connections = {'b_salir/clicked': self.salir,
            ...
            'b_cambiar_producto_compra/clicked': self.cambiar_producto_compra, 
            'b_add_producto_a_consumo/clicked': self.add_producto_a_consumo
            }
        self.add_connections(connections)
        cols = (("Descripción (opcional)", "gobject.TYPE_STRING", 
                    True, True, True, None),
                ("Material", "gobject.TYPE_STRING", 
    ...
    def guardar(self, b):
        """
        Guarda los valores para la cantidad en los campos de los registros 
        correspondientes.
        """
        try:
            self.cas['ensimaje'].cantidad = float(
                self.wids['e_censimaje'].get_text())
        except:
            utils.dialogo_info(titulo = "ERROR DE FORMATO", 
                texto = "Corrija el formato numérico usado y vuelva a "
                        "intentarlo.", 
                padre = self.wids['ventana'])
        try:
            self.cas['antiuvi'].cantidad = float(
                self.wids['e_cantiuvi'].get_text())
        except:
            utils.dialogo_info(titulo = "ERROR DE FORMATO", 
                texto = "Corrija el formato numérico usado y vuelva a "
                        "intentarlo.", 
                padre = self.wids['ventana'])
        try:
            self.cas['negro'].cantidad = float(
                self.wids['e_cnegro'].get_text())
        except:
            utils.dialogo_info(titulo = "ERROR DE FORMATO", 
                texto = "Corrija el formato numérico usado y vuelva a "
                        "intentarlo.", 
                padre = self.wids['ventana'])
        try:
            self.cas['titanio'].cantidad = float(
                self.wids['e_ctitanio'].get_text())
        except:
            utils.dialogo_info(titulo = "ERROR DE FORMATO", 
                texto = "Corrija el formato numérico usado y vuelva a "
                        "intentarlo.", 
                padre = self.wids['ventana'])
        try:
            self.cas['plástico grueso'].cantidad = float(
                self.wids['e_cplasticog'].get_text())
        except:
            utils.dialogo_info(titulo = "ERROR DE FORMATO", 
                texto = "Corrija el formato numérico usado y vuelva a "
                        "intentarlo.", 
                padre = self.wids['ventana'])
        try:
            self.cas['plástico estirable'].cantidad = float(
                self.wids['e_cplasticoe'].get_text())
        except:
            utils.dialogo_info(titulo = "ERROR DE FORMATO", 
                texto = "Corrija el formato numérico usado y vuelva a "
                        "intentarlo.", 
                padre = self.wids['ventana'])
        try:
            self.cas['flejes'].cantidad = float(
                self.wids['e_cflejes'].get_text())
        except:
            utils.dialogo_info(titulo = "ERROR DE FORMATO", 
                texto = "Corrija el formato numérico usado y vuelva a "
                        "intentarlo.", 
    ...
    def rellenar_consumos_adicionales_por_producto(self):
        ...
        consumos = pclases.ConsumoAdicional.select("""
            ...
            """, orderBy = "id")
        linea_cemento = self.linea
        for consumo in consumos:
            if not linea_cemento in consumo.formulacion.lineasDeProduccion:
                continue    # Me salto los consumos que son exclusivamente de 
                            # fibra no embolsada (balas o bigbags). 
    ...
    def comprobar_registro(self):
        ...
        de embolsado y los registros de descuento automático relacionados.
        Si no existen, los crea con valores por defecto.
        """
        try:
            self.linea = pclases.LineaDeProduccion.select(
                pclases.LineaDeProduccion.q.nombre.contains("embolsa"))[0]
        except:
            utils.dialogo_info(titulo = "ERROR GRAVE", 
                texto = "No se encontró la línea de embolsado en la BD."\
                        "\nCierre la ventana y contacte con el administrador"\
                        " de la aplicación.", 
                padre = self.wids['ventana'])
            return
        self.objeto = self.linea.formulacion
        if self.objeto == None:
            txt = "formulacion_bolsas.py::comprobar_registro -> Formulación "\
                  "de embolsado no existe. Creando..."
            print txt
        ...
        nombres_ca = {'palé': (1, ' ud / ud'), 
                      'caja': (pclases.Pale.NUMCAJAS, ' ud / ud'), 
                        # Por defecto, NUMCAJAS; pero no es lo que se 
                        # guardará en los consumos que se crean en el 
                        # siguiente bucle.
                     }
        self.cas = {}
        productos_embolsado = pclases.ProductoVenta.select(
            ...
            if nombre not in nombres_ca_existentes:
                ...
                for productoVenta in productos_embolsado:
                    ...
                    if ceb == cantidad_defecto:
                        # Es de 14 cajas por palé. No pasa nada.
                        ca.addProductoVenta(productoVenta)
                    else:   # Tengo que crear un consumo adicional que chupe 
                            # el número de cajas que dice el producto.
                        print "Creando nuevo."
                        ...
                        try:
                            ...
                            nca = ca.clone(cantidad = ceb.cajasPale, 
                                           nombre = nuevonombre)
                        nca.addProductoVenta(productoVenta)
                        self.cas[nuevonombre] = nca
                self.cas[nombre] = ca
            else:
                self.cas[nombre] = [ca 
                                    for ca in self.objeto.consumosAdicionales 
                                    if ca.nombre == nombre][0]
    ...
    def add_consumo_por_producto(self, boton):
        ...
        if productos:
            ...
            if productos != None and len(productos) > 0 and producto_compra != None:
    ...
    def pedir_cantidad(self):
        ...
        res = utils.dialogo_entrada(titulo = "CANTIDAD", 
                texto = "Introduzca la cantidad a consumir del producto de "
                        "compra (sin unidades):", 
                padre = self.wids['ventana'])
        try:
