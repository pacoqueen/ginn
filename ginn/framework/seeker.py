...
class Seeker:
    ...
    def __construir_criterio_consulta(self, clase):
        ...
        """
        criterios = []
        for nombre_campo in list(clase.sqlmeta.columns.keys()):
...
class VentanaGenerica(Ventana):
        ...
        if isinstance(col, pclases.SOStringCol):
            ...
            if isinstance(widget, gtk.RadioButton):
                for w in widget.get_group():
                    w.set_inconsistent(valor is None)
                # list(map(lambda w: w.set_inconsistent(valor is None),
                #     widget.get_group()))
                if valor == False:
            ...
            # Lo intento como puedo. A lo mejor faltaría intentarlo también como si fuera un TextView.
            if hasattr(widget, "child"):
                widget.child.set_text(repr(valor))
            else:
                widget.set_text(repr(valor))
    ...
    def rellenar_tabla(self, coljoin):
        ...
        columnas = list(getattr(coljoin.otherClass, "sqlmeta.columns").keys())
        columnas.append('id')
        model.clear()
