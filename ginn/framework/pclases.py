...
#except:
#    import sys
#    sys.path.append('../SQLObject/SQLObject-0.6.1/')
#    from sqlobject import *
# El SQLObject de Guadalinex-Ubuntu (creo que el de debian no) tiene un BUG
# con los MultipleJoin e ignora el nombre definido para poner el de la 
...
def starter(objeto, *args, **kw):
    ...
    objeto.notificador = framework.notificacion.Notificacion(objeto)
...
def print_verbose(cont, total, antes):
    if VERBOSE:
        ...
        print "Cargando... (%d/%d) Tiempo restante estimado: %d seg" % (
            cont, total, estimado)
...
class HistorialExistencias(SQLObject, PRPCTOO, CacheExistencias):
    ...
    def test(self, almacen = None):
        ...
        total_bultos = (   existencias_base 
                        ...
                        +   consumos_bultos)
        if DEBUG:
            print "[", self.productoVenta.id, "]", self.productoVenta.descripcion 
            print "\tBULTOS:", utils.str_fecha(hoy), existencias_base
            print "\tPRODUCCIÓN:", utils.str_fecha(fecha), "->", utils.str_fecha(hoy), "-", produccion_bultos
            print "\tCONSUMOS:", utils.str_fecha(fecha), "->", utils.str_fecha(hoy), "+", consumos_bultos
            print "\tVENTAS:", utils.str_fecha(fecha), "->", utils.str_fecha(hoy), "+", ventas_bultos
            print "\t\t\t", "=", total_bultos
            print "\tCACHEADO", utils.str_fecha(self.fecha), ":", self.bultos, " - ", self.bultos == total_bultos and "OK" or "KO"
        ##### CANTIDAD #####
        ...
        if DEBUG:
            print "[", self.productoVenta.id, "]", self.productoVenta.descripcion
            print "\tCANTIDAD:", utils.str_fecha(hoy), stock_base
            print "\tPRODUCCIÓN:", utils.str_fecha(fecha), "->", utils.str_fecha(hoy), "-", produccion_cantidad
            print "\tCONSUMOS:", utils.str_fecha(fecha), "->", utils.str_fecha(hoy), "+", consumos_cantidad
            print "\tVENTAS:", utils.str_fecha(fecha), "->", utils.str_fecha(hoy), "+", ventas_cantidad
            print "\t\t\t", "=", total_cantidad
            print "\tCACHEADO", utils.str_fecha(self.fecha), ":", self.cantidad, " - ", self.cantidad == total_cantidad and "OK" or "KO"
...
class Silo(SQLObject, PRPCTOO):
    ...
    def consumir(self, cantidad, parte_de_produccion = None):
        ...
        while int(cantidad_ocupada) > 0 and int(cantidad) > 0:
            ...
            if DEBUG:
                try:
                    print " --->", carga_antigua.cantidad, carga_antigua.consumido, cantidad_de_producto
    ...
    def get_carga_mas_antigua(self):
        ...
        if int(cargado) > 0:
            for carga_mas_antigua in cargas:
                ...
                if  int(cargado) <= 0:
                    break
        if DEBUG: print " --> ", carga_mas_antigua and carga_mas_antigua.cantidad or " None"
    ...
    def actualizar_estado_pago_domiciliaciones():
        ...
        for v in VencimientoPago.select(AND(
            ...
            if mx.DateTime.today() >= v.fecha:
                if DEBUG:
                    print "Actualizando el estado de %s..." % v.get_puid(),
                    try:
                        sys.stdout.flush()
                    except AttributeError:
                        pass    # Consola de depuración o algo. No tiene flush.
...
class LoteCem(SQLObject, PRPCTOO):
    ...
    def update_valor(self, caracteristica):
        ...
        try:
            ...
            print "pclases::class LoteCem::update_valor-> El atributo %s no es correcto." % (caracteristica)
...
class Caja(SQLObject, PRPCTOO):
    ...
    def get_bolsas(self):
        ...
        try:
            ...
            print "El palé %s tiene un producto de venta relacionado "\
...
class PartidaCarga(SQLObject, PRPCTOO):
    ...
    def crear_albaran_interno(self):
        ...
        if cliente == None:
            print "pclases.py::class PartidaCarga::crear_albaran_interno-> No se pudo encontrar propia empresa como cliente."
            albaran = None
        else:
...
class Partida(SQLObject, PRPCTOO):
    ...
    def get_producto(self):
        ...
        if productocount == 0:
            ...
            print "pclases::Partida::get_producto -> %d productos encontrados para la partida ID %d. Devuelvo el primero de ellos." % (productocount, self.id)
            return producto[0]
        else:
            return producto[0]
    ...
    def entra_en_cota_superior(self, fecha, 
        ...
        if contar_partida_carga_completa:
            if self.partidaCarga != None:
                ...
                print "pclases::Partida::entra_en_cota_superior -> La partida ID %d no tiene partida de carga. Ignoro parámetro 'contar_partida_carga_completa." % (self.id)
        return res
    ...
    def entra_en_cota_inferior(self, fecha, contar_partida_carga_completa = True):
        ...
        if contar_partida_carga_completa:
            if self.partidaCarga != None:
                ...
                print "pclases::Partida::entra_en_cota_inferior -> "\
    ...
    def get_existencias_historico(self, 
        ...
        if res == None:  # Aún no he encontrado las existencias, bien por 
            ...
            if almacen:
                ...
                res = sum([caches_almacenes])
        if DEBUG:
            print "pclases::get_existencias_historico -> self:", 
            print self.descripcion, "[", self.get_puid(), "]", 
            print "| res:", res, "| fecha:", fecha, "| forzar:", forzar, 
            print "| almacen:", almacen and almacen.nombre or "-"
        return res
...
class Articulo(SQLObject, PRPCTOO):
    ...
    def get_analizado(self):
        ...
        if self.es_bala():
            ...
            print "Artículo ID %d no es bala [de cable], rollo [{defectuoso,"\
    ...
    def get_codigo_interno(self):
        ...
        print "ERROR: pclases.py: get_codigo_interno: El artículo no tiene bala [cable], rollo [{defectuoso, C}] ni bigbag relacionado."
        return None
    ...
    def get_fechahora(self):
        ...
        if self.es_rollo():
            ...
            print "ERROR: pclases.py: get_fechahora: ¡Artículo no es ni bala "\
    ...
    def es_de_baja_calidad(self):
        ...
        if self.es_rollo():
            ...
            # NO DEBERÍA PASAR.
            print "ERROR: pclases.py: es_de_baja_calidad: ¡Artículo no es ni "\
    ...
    def comparar_con_marcado(self, valor, prueba, fecha = None):
        ...
        if valor != None:
            ...
            if inf <= optimo <= sup:    # Inf SIEMPRE SIEMPRE es menor que 
                ...
                print "pclases::CamposEspecificosRollo::comparar_con_marcado"\
...
class ProductoVenta(SQLObject, PRPCTOO, Producto):
    ...
    def buscar_produccion_bultos(self, fecha0, fecha1):
        ...
        if (pv.es_rollo() or pv.es_bala() or pv.es_bigbag() or pv.es_caja()):
            ...
            print "pclases::ProductoVenta: ProductoVenta ID %d no es bala [cable], rollo [C] ni bigbag." % (pv.id)
        return res
    ...
    def buscar_produccion_cantidad(self, fecha0, fecha1):
        ...
        if pv.es_bala():
            ...
            print "pclases::ProductoVenta: ProductoVenta ID %d no es bala [cable], rollo [C] ni bigbag ni caja de fibra de cemento." % (pv.id)
        return res
                ...
                for a in alb.articulos:
                    if a.productoVenta == self:
                        if (a.es_bala() or a.es_bala_cable() 
                            ...
                            print "pclases::ProductoVenta::buscar_ventas_cant"\
...
--              AND es_interno(albaran_salida.id) = FALSE 
        ...
        if fecha:
            if almacen != None:
                ...
                if historial.count() > 1:
                    print "pclases.py: _buscar_en_historico: Caché corrupta"\
    ...
    def get_existencias(self, hasta = None, forzar = False, actualizar = True, 
        ...
        if not almacen:
            try:
                ...
                if DEBUG:
                    print "pclases.py::ProductoVenta.get_existencias: No hay "\
    ...
    def get_salidas(self, desde, contar_defectuosos = False, almacen = None, 
        ...
        if DEBUG:
            print "# artículos en albaranes de salida:", articulos_as.count()
    ...
    def get_entradas(self, desde, contar_defectuosos = False, almacen = None, 
        ...
        if DEBUG:
            print "# artículos en partes de producción:", articulos_pdp.count()
        # 3.- Abonos
        AB = Abono
        LDD = LineaDeDevolucion
        clauses = []
        ...
        if DEBUG:
            print "# artículos abonados:", articulos_abonados.count()
    ...
    def _OBSOLETE_get_stock(self, hasta = None, forzar = False, 
        ...
        if historicos:
            ...
            if self.es_bala():
                    ...
                    if hasta:
                        ...
                        rollos_defectuosos_almacen = RolloDefectuoso.select(
                            AND(RolloDefectuoso.q.id 
                                ...
                                Articulo.q.productoVentaID == self.id, 
                                Articulo.q.albaranSalidaID == None))
                    if DEBUG: print "Antes de defectuosos:", cantidad
                    cantidad += sum([rd.ancho * rd.metrosLineales for rd in rollos_defectuosos_almacen])
                    if DEBUG: print "Después de defectuosos:", cantidad
            elif self.es_bigbag():
                try:
                    if not hasta:
                ...
                print "pclases::ProductoVenta::get_stock: ERROR: El producto de venta %s no es ni rollo ni bigbag ni caja ni bala de cable. Verificar." % (self.id)
                cantidad = 0.0
            if actualizar:
    ...
    def get_stock(self, hasta = None, forzar = False, actualizar = True, 
        ...
        if historicos:
            ...
            if self.es_bala():
                ...
                print "pclases::ProductoVenta::get_stock: ERROR: El producto "\
    ...
    def get_existencias_A(self, hasta = None, forzar = False, 
        ...
        if not hasta or hasta >= mx.DateTime.localtime():
            ...
            if not almacen:
                ...
                if DEBUG:
                    print "pclases::ProductoVenta:get_existencias_A "\
                          "-> almacen != None."
                if forzar:
    ...
    def get_partidas(self, fechaini = None, fechafin = None):
        ...
        if fechaini:
            ...
            for p in res:
                ...
                if DEBUG:
                    print "pclases::ProductoVenta.get_partidas", \
        ...
        if DEBUG:
            print "pclases::ProductoVenta.get_partidas", len(res)
        return res
...
class ConsumoAdicional(SQLObject, PRPCTOO):
    ...
    def consumir(self, 
        ...
        if producto_a_consumir == None:
            print "pclases.py: class ConsumoAdicional::consumir(): ¡El producto a consumir es None! Ignoro consumo y devuelvo 0 como cantidad consumida."
            return 0
    ...
    def _calcular_cantidad_a_consumir(self, 
        ...
        if unidad == ConsumoAdicional.PORCENTAJE:
            ...
            if self.productoCompra.unidad.strip().lower().replace(".", "") != "kg":
                print 'WARNING pclases.py (_calcular_cantidad_a_consumir): La unidad del producto de compra %s [%d] (%s) no coincide con "kg", que es la unidad de consumo en la formulación.' % (self.productoCompra.descripcion, self.productoCompra.id, self.productoCompra.unidad)
            ...
            if self.productoCompra.unidad.strip().lower().replace(".", "") != unidad_pc.strip().lower().replace(".", ""):
                print 'WARNING pclases.py (_calcular_cantidad_a_consumir): La unidad del producto de compra %s [%d] (%s) no coincide con "%s", que es la unidad de consumo en la formulación.' % (self.productoCompra.descripcion, self.productoCompra.id, self.productoCompra.unidad, unidad_pc)
            if unidad_pv.strip().lower() == "ud":
                ...
                print "ERROR pclases.py (_calcular_cantidad_a_consumir): No se pudo interpretar la unidad de consumo de la formulación. ConsumoAdicional.id = %d" % (self.id)
                cantidad = 0
        return cantidad
    ...
    def crear_alarmas_automaticas(fras_de_venta):
        ...
        for fra in fras_de_venta:
            ...
            if (fra.calcular_pendiente_de_documento_de_pago() 
                ...
                if DEBUG:
                    print "Creando alarma automática..."
                ...
                if DEBUG:
                    print 'Alarma "%s" con fecha de alarma %s creada.' % (
    ...
    def esta_en_tarifa(self, producto):
        ...
        if isinstance(producto, ProductoVenta):
            ...
            if precios.count() == 1:
                ...
                print "WARNING: pclases.py: obtener_precio: Más de un precio para una misma tarifa y producto de venta."
                return True
        elif isinstance(producto, ProductoCompra):
...
class ParteDeProduccion(SQLObject, PRPCTOO):
    ...
    def crear_albaran_interno(self):
        ...
        cliente = DatosDeLaEmpresa.get_cliente()
        if cliente == None:
            print "pclases.py::class ParteDeProduccion::crear_albaran_interno-> No se pudo encontrar propia empresa como cliente."
            albaran = None
        else:
            #numalbaran = AlbaranSalida.get_ultimo_numero_numalbaran() + 1
    ...
    def calcular_productividad(self):
        ...
        try:
            ...
            print "pclases::ParteDeProduccion::calcular_productividad -> Parte ID %d con más tiempo de paradas que duración. AssertionError: %s" % (self.id, msg)
            ht = 0.0
        try:
    ...
    def se_solapa(self, desbloquear_si_mal = False):
        ...
        if DEBUG:
            for p in partes:
                print p.id, p.fecha, p.horainicio, p.horafin, \
                      p.fechahorainicio, p.fechahorafin
            print "------------------------------"
        res = partes.count() > 1    # Sólo debo aparecer yo (self) en el 
                                    # resultado de la consulta.
        if DEBUG and res:
            print [(pdp.id, pdp.fechahorainicio, pdp.fechahorafin) 
            ...
            if horas == None:
                try:
                    ...
                    try:
                        ...
                        if DEBUG:
                            print "pclases::ParteDeProduccion.addEmpleado "\
    ...
    def get_cantidad(self):
        ...
        if a != None:
            if a.es_rollo() or a.es_rollo_defectuoso():
                ...
                res = 0
                print "Artículo ID %d no es bala [cable], cemento, rollo, rolloC o rollo_defectuoso." % (a.id)
        ...
        for join in malo._SO_joinDict:
            ...
            for dependiente in getattr(malo, lista):
                ...
                try:
                    ...
                    print "pclases.py::unificar -> UnboundLocalError %s:"\
                          "\ndependiente: %s\ncol: %s" % (
                            e, dependiente, col)
