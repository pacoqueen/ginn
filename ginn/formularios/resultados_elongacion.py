...
###################################################################
## resultados_elongacion.py - Resultados de pruebas de elongación. 
###################################################################
## NOTAS:
##  
###################################################################
...
class ResultadosElongacion(Ventana):
    def __init__(self, objeto = None, usuario = None):
        ...
        """
        Ventana.__init__(self, 'resultados_elongacion.glade', objeto, usuario = usuario)
    ...
    def rellenar_pruebas(self):
        ...
        pruebas = pclases.PruebaElongacion.select(pclases.PruebaElongacion.q.loteID == self.lote.id)
    ...
    def add(self, w):
        if self.lote != None:
            ...
            if resultado == '':
                ...
                return
            try:
                prueba = pclases.PruebaElongacion(fecha = utils.parse_fecha(fecha),
                                                 resultado = resultado,
                                                 lote = self.lote)
    ...
    def cambiar_fecha(self, cell, path, texto):
        model = self.wids['tv_pruebas'].get_model()
        prueba = pclases.PruebaElongacion.get(model[path][-1])
        try:
