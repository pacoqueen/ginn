...
from formularios.ventana_progreso import VentanaProgreso, VentanaActividad
...
class AlbaranesDeSalida(Ventana):
    def __init__(self, objeto = None, usuario = None):
        ...
        if self.objeto and not isinstance(self.objeto, pclases.AlbaranSalida):
            # Debe ser un número de albarán. Lo intento o desisto.
            try:
                self.objeto = pclases.AlbaranSalida.selectBy(
                        numalbaran = self.objeto)[0]
            except IndexError:
                try:
                    self.objeto = pclases.AlbaranSalida.get(int(self.objeto))
        ...
        if pclases.DEBUG:
            antes = time.time()
            myprint("Voy a inicializar la ventana...")
        self.inicializar_ventana()
        if pclases.DEBUG:
            myprint("    ... ventana inicializada. ", time.time() - antes)
        if self.objeto == None:
            self.ir_a_primero()
        else:
    ...
    def resaltar_credito(self, combo):
        ...
        if idcliente:
            ...
            if credito == sys.maxint:   # ¿maxint, te preguntarás? Ver
                ...
                if credito <= 0:
                    color = combo.child.get_colormap().alloc_color(
                                                                "IndianRed1")
                else:
                    color = None
        ...
        if res == []:
            utils.dialogo_info(titulo = "ERROR CÓDIGO",
                texto = "El texto %s no es un código válido.\nSi está "
                        "intentando introducir productos duplicados por "
                        "motivos excepcionales\n(su código acaba en D) no "
                        "pueden ser añadidos por lote.\n\nIntrodúzcalos uno a "
                        "uno usando el código de trazabilidad completo,\n"
            ...
            try:
                ...
                self.asociar_articulos_a_albaran([articulo])
    ...
    def pedir_rango(self, producto):
        ...
        Por ejemplo:
            123-145     Intentará añadir los rollos 123 a 145, ambos inclusive.
                        Si no los encuentra, los buscará entre las balas de
                        fibra.
            R123-145    Añadirá los rollos 123 a 145, ambos inclusive.
            R123-R145   Hace lo mismo que el caso anterior.
            B123-145    Añadirá, si se encuentran y están debidamente
                        analizadas y catalogadas, las balas de fibra 123 a 145,
                        ambas inclusive.
            B123-B145   Hace lo mismo que en el caso anterior.
            C10-C15     BigBags de GEOCEM del número 10 al 15.
            B100-B105, R4000 C101   Introduce el rango de balas de 100 a 105,
                        ambas inclusive; el rollo 4000 y el bigbag 101.
                ...
                try:
                    for codigo in rangocodigos:
                        ...
                        if len_antes == len_despues:
                            ...
                            # incorrecto.
                            # Voy a preguntar si quiere seguir, porque como
                            # haya metido un rango de 1000 códigos y no esté
                            # ninguno, el usuario se va a hinchar de darle a
                            # aceptar.
                            txt = """
        ...
        if (articulos_baja_calidad == []
            or utils.dialogo(titulo = "ARTÍCULOS DE BAJA CALIDAD",
                             texto = texto,
                             padre = self.wids['ventana'])):
            self.asociar_articulos_a_albaran(articulos)
            self.objeto.calcular_comisiones()
            self.actualizar_ventana()
    ...
    def add_producto(self, codigo, articulos, tipocodigo = ""):
        ...
        if tipocodigo == "":
            ...
            articulo = pclases.Bigbag.select(
                        pclases.Bigbag.q.numbigbag == codigo)
            if articulo.count() == 0:
                return articulos    # No lo encuentro, devuelvo sin añadir nada
        elif tipocodigo == "Z": # Sólo busco cable de fibra.
            articulo = pclases.BalaCable.select(
                    pclases.BalaCable.q.codigo == "Z%d" % codigo)
            if articulo.count() == 0:
                return articulos    # No lo encuentro, devuelvo sin añadir nada
        ...
        #        return articulos  # No lo encuentro, devuelvo sin añadir nada.
            ...
            self.logger.error("albaranes_de_salida.py. Se solicitó añadir "
                              "artículos que no son balas, rollos, rollos "
                              "defectuosos, balas de cable ni geocem. "
    ...
    def es_diferente(self):
        ...
        if pclases.DEBUG and not condicion:
            myprint("numalbaran", albaran.numalbaran)
        condicion = condicion and (utils.str_fecha(albaran.fecha)
                                    == self.wids['e_fecha'].get_text())
        if pclases.DEBUG and not condicion:
            myprint("fecha", albaran.fecha)
        ...
        if pclases.DEBUG and not condicion:
            myprint("cliente", albaran.cliente)
        condicion = condicion and (self.wids['ch_facturable'].get_active()
                                                    == self.objeto.facturable)
        if pclases.DEBUG and not condicion:
            myprint("facturable", albaran.facturable)
        condicion = (condicion
            and self.wids['e_motivo'].get_text() == self.objeto.motivo)
        if pclases.DEBUG and not condicion:
            myprint("motivo", albaran.motivo)
        condicion = (condicion
            and self.wids['ch_bloqueado'].get_active() == albaran.bloqueado)
        if pclases.DEBUG and not condicion:
            myprint("bloqueado", albaran.bloqueado)
        condicion = (condicion
                and self.wids['cbe_nom'].child.get_text() == albaran.nombre)
        if pclases.DEBUG and not condicion:
            myprint("nombre", albaran.nombre)
        condicion = condicion and self.wids['e_cp'].get_text() == albaran.cp
        if pclases.DEBUG and not condicion:
            myprint("cp", albaran.cp)
        condicion = (condicion
                and self.wids['e_ciudad'].get_text() == albaran.ciudad)
        if pclases.DEBUG and not condicion:
            myprint("ciudad", albaran.ciudad)
        condicion = (condicion
                and self.wids['e_pais'].get_text() == albaran.pais)
        if pclases.DEBUG and not condicion:
            myprint("pais", albaran.pais)
        condicion = (condicion
                and self.wids['e_telf'].get_text() == albaran.telefono)
        if pclases.DEBUG and not condicion:
            myprint("telefono", albaran.telefono)
        condicion = (condicion
                and self.wids['e_direccion'].get_text() == albaran.direccion)
        if pclases.DEBUG and not condicion:
            myprint("direccion", albaran.direccion)
        buff = self.wids['tv_observaciones'].get_buffer()
        condicion = (condicion and
                buff.get_text(buff.get_start_iter(), buff.get_end_iter())
                    == albaran.observaciones)
        if pclases.DEBUG and not condicion:
            myprint("observaciones", albaran.observaciones)
        condicion = (condicion and utils.combo_get_value(self.wids['cbe_dni'])
                                                    == albaran.transportistaID)
        if pclases.DEBUG and not condicion:
            myprint("transportista", albaran.transportista)
        condicion = (condicion and utils.combo_get_value(self.wids['cbe_nom'])
                                                    == albaran.destinoID)
        if pclases.DEBUG and not condicion: myprint("destino", albaran.destino)
        condicion = (condicion and
        ...
        if isinstance(producto, pclases.ProductoVenta):
            ...
            if producto.es_caja():
                ...
                for a in utils.unificar(articulos_anadidos):
                    if a.es_rollo():
                        cer = a.productoVenta.camposEspecificosRollo
                        metros_lineales = cer.metrosLineales
                        ancho = cer.ancho
                        cantidad += metros_lineales * ancho
                    elif a.es_bala():
                        cantidad += a.bala.pesobala
                    elif a.es_bigbag():
                        cantidad += a.bigbag.pesobigbag
    ...
    def ir_a_primero(self):
        ...
        try:
            ...
            albaran = pclases.AlbaranSalida.select(orderBy = '-id')[0]
                # Selecciono todos los albaranes de venta y me quedo con el
                # primero de la lista.
    ...
    def colorear(self, ldvs):
        ...
        def cell_func(column, cell, model, itr, data):
            ...
            # NOTA: Esto hay que hacerlo porque el nuevo cell_func machaca el
            #       predefinido por defecto en el utils.preparar_x
        ...
        for i in xrange(len(cols)):
            ...
            for cell in cells:
                if not isinstance(cell, gtk.CellRendererPixbuf):    # Para no
                                    # machacar el data_func del icono "notas".
                    column.set_cell_data_func(cell,cell_func, [i, ldvs])

    def rellenar_ldvs(self, albaran):
        if pclases.DEBUG:
            myprint("Soy rellenar_ldvs."
                    "Hasta ahora mismo no he sido invocada.")
    ...
    def crear_nuevo_albaran(self, widget):
        ...
        """
        albaran = self.objeto
        nuevo_num = pclases.AlbaranSalida.get_siguiente_numero_numalbaran_str()
            # Datos a pedir:
        numalbaran = utils.dialogo_entrada(titulo = "NÚMERO DE ALBARÁN",
            ...
            valor_por_defecto = nuevo_num,
            padre = self.wids['ventana'])
        if numalbaran == None:
            return
        numero_numalbaran_usuario = utils.parse_numero(numalbaran,
                                        invertir = True)
        numero_numalbaran_sugerido = utils.parse_numero(nuevo_num,
                                        invertir = True)
        if (self.usuario
            and self.usuario.nivel > 2
    ...
    def add_pedido(self, boton):
        ...
        if pclases.DEBUG:
            myprint("albaranes_de_salida -> importe_pedido", importe_pedido)
        ...
        if pedido.cerrado:
            ...
            if pedido.cliente != albaran.cliente:
                ...
                if not_included:
                    ...
                    # TODO: Para pedidos con muchos albaranes, la lista de
                    # números de albarán es demasiado larga y el diálogo
                    # sobrepasa el ancho de la pantalla.
                    txtnoalb = """
    ...
    def asociar_articulos_a_albaran(self, articulos):
        """
        Verifica que los artículos pertenezcan a una LDV existente.
        Recibe una lista de artículos que deben ser del mismo producto de
        venta. Verifica también que el artículo no esté en otro albarán de
        salida y que haya sido analizado.
        """
        if len(articulos) == 0:
            return
        albaran = self.objeto
    ...
    def desvincular_ldv_del_albaran(self, ldv):
        ...
        ajustar_existencias(ldv, 2 * ldv.cantidad)
            # Le paso el doble como cantidad anterior para que al restar quede
            # en positivo e incremente la cantidad
        ldv.albaranSalida = None
        if ldv.facturaVentaID == None and ldv.prefacturaID == None:
            try:
                ldv.destroy(ventana = __file__)
            except:
                txterror = "albarabes_de_salida::desvincular_ldv_del_albaran "\
                           "-> La LDV ID %d no tiene albarán ni factura(s)"\
                           "pero no se pudo eliminar." % (ldv.id)
                myprint(txterror)
    ...
    def borrar_albaran(self, boton):
        ...
        try:
            ...
            utils.dialogo_info(titulo = 'ERROR',
                    texto = 'No se pudo eliminar.\nIntente eliminar primero '
                            'los productos, servicios, transportes y '
                            'comisiones del albarán.',
                    padre = self.wids['ventana'])
            return
        self.ir_a_primero()
        ...
        for producto in prods:
            ...
            if hasattr(producto, "es_caja") and producto.es_caja():
                ...
                try:
                    ...
                    myprint("albaranes_de_salida.py::imprimir ->", msg)
                    myprint(bultos)
                    bultospales = []
                    for ldv in prods[producto]:
            ...
            if es_venta_rollos_c(prods[producto][0]):
                ...
                try:
                    ...
                    txterror = "albaranes_de_salida::"\
                               "preparar_llamada_imprimir -> "\
    ...
    def get_numpedidos_ldvs(self, ldvs):
        ...
        for ldv in ldvs:
            if isinstance(ldv, pclases.LineaDeVenta):
                if ldv.pedidoVenta and ldv.pedidoVenta.numpedido not in nps:
    ...
    def imprimir(self, w):
        ...
        if pclases.DEBUG:
            ...
            antes = time.time()
        vpro.mover("(Redistribuyendo cantidades)")
        self.preguntar_si_redistribuir()
        vpro.mover("(Guardando cambios)")
        if pclases.DEBUG:
            myprint("    --> Sin hueso mi ansiedad:", time.time() - antes)
            myprint("Llamando a self.guardar...")
            antes = time.time()
        self.guardar(None, actualizar_ventana = False)  # Si se ha olvidado
            # guardar, guardo yo.
        vpro.mover("(Comprobando transportes)")
        if pclases.DEBUG:
            myprint("    --> Sin hueso mi ansiedad:", time.time() - antes)
            myprint("Comprobando self.wids['ch_debellevargastos']...")
            antes = time.time()
        albaran = self.objeto
        if (self.wids['ch_debellevargastos'].get_active()
            ...
            utils.dialogo_info(titulo = "ALBARÁN INCOMPLETO",
                ...
                padre = self.wids['ventana'])
            return
        vpro.mover("(Ajustando existencias)")
        if pclases.DEBUG:
            myprint("    --> Sin hueso mi ansiedad:", time.time() - antes)
        ...
        if pclases.DEBUG:
            myprint("    --> Sin hueso mi ansiedad:", time.time() - antes)
            myprint("Llamando a actualizar_ventana...")
            antes = time.time()
        self.actualizar_ventana()
            # Para refrescar los cambios en cantidades autoajustadas y tal.
        vpro.mover("(Preparando vista previa albarán)")
        if pclases.DEBUG:
            myprint("    --> Sin hueso mi ansiedad:", time.time() - antes)
            myprint("Llamando a preparar_llamada_imprimir...",
                    antes = time.time())
        c, f, e, g, l, o, d, t, cT, cD=self.preparar_llamada_imprimir(albaran)
        if c == f == e == g == None:    # etc...
            return
        vpro.mover("(Comprobando configuración multipágina)")
        if pclases.DEBUG:
            myprint("    --> Sin hueso mi ansiedad:", time.time() - antes)
            myprint("Comprobando configuración de albarán multipágina...")
            antes = time.time()
        ...
        vpro.mover("(Calculando comisiones)")
        if pclases.DEBUG:
            myprint("    --> Sin hueso mi ansiedad:", time.time() - antes)
            myprint("Llamando a self.rellenar_comisiones...")
            antes = time.time()
        self.rellenar_comisiones()
        vpro.mover("(Generando carta de portes y CMR)")
        if pclases.DEBUG:
            myprint("    --> Sin hueso mi ansiedad:", time.time() - antes)
            myprint("A partir de aquí ya son movidas de carta de portes, CMR, "
        ...
        if pclases.config.get_carta_portes():
            ...
            recogida = utils.dialogo(titulo = "¿IMPRIMIR HOJA DE RECOGIDA?",
                        texto = "Responda «Sí» para generar una hoja adicional"
                                " de recogida de envases vacíos.",
                        padre = self.wids['ventana'],
                        defecto = gtk.RESPONSE_NO,
                        tiempo = 10)
    ...
    def imprimir_cmr(self):
        lugar_entrega = utils.dialogo_entrada(titulo = "CMR",
        ...
        if lugar_entrega != None:
            transportista = utils.dialogo_entrada(titulo = "CMR",
                                                  ...
                                                  textview = True)
            if transportista != None:
                porteadores = utils.dialogo_entrada(titulo = "CMR",
                                    texto = "Porteadores:",
                                    padre = self.wids['ventana'],
                                    textview = True)
                if porteadores != None:
                    abrir_pdf(geninformes.cmr(self.objeto,
                                              lugar_entrega,
                                              transportista,
                                              porteadores))

    def enviar_por_correo(self, email, fichero_albaran=None,
                          fichero_factura=None, fichero_packing_list=None):
        """
        ...
        if adjuntos == []:
            ...
            texto = "Adjunto la siguiente documentación en formato PDF "\
                    "correspondiente al albarán %s%s" % (
                            self.objeto.numalbaran, texto_adjuntos)
        ...
        if not ok:
            utils.dialogo_info(titulo = "ERROR ENVÍO E-MAIL",
                               texto = "Ocurrió un error enviando el correo "
                                       ...
                                       "e inténtelo más tarde desde su propio"
                                       " cliente de correo.",
                               padre = self.wids['ventana'])
        else:
            utils.dialogo_info(titulo = "CORREO ELECTRÓNICO ENVIADO",
                               texto = "Se envío el correo electrónico a los "
                                       "destinatarios y una copia al "
                                       "remitente.\n"
                                       "Verifique que lo recibe y vuelva a "
                                       "enviar la documentación en caso "
                                       "contrario.",
                               padre = self.wids['ventana'])
    ...
    def packinglist(self, boton, abrir_pdf = True):
        ...
        for ldv, linea_de_venta in [(self.__ldvs[ide],
                        ...
                        if pclases.LineaDeVenta.get(ide).productoVenta]:
        ...
        for a in articulos:
            if a.es_bala():
                ...
                try:
                    ...
                    self.logger.error("albaranes_de_salida.py:"
                            " get_balas_o_rollos: (packing list). El producto"
                            " de venta del artículo no tiene campos"
                            " específicos o el artículo no tiene producto de"
                            " venta relacionado.")
                    metros2 = 0
                ...
                self.logger.error("El artículo ID %d no tiene asociado ni una"
                        " bala, ni rollo [defectuoso] ni bigbag." % (a.id))
        balas_aux = []
        i = 0
        ...
        for articulo in articulos:
            if articulo.productoVenta.es_bala():
                try:
                    ...
                    self.logger.error("El artículo ID %d es fibra pero no "
                                      "tiene lote." % (articulo.id),
                                      exc_info = True)
                    codlote = None
                ...
                try:
                    ...
                    self.logger.error("El artículo ID %d es geotextil pero no"
                                      " tiene partida." % (articulo.id),
                                      exc_info = True)
                    codlote = None
                if codlote != None and codlote not in lotes:
                    lotes.append(codlote)
                ...
                try:
                    ...
                    self.logger.error("El artículo ID %d es geocem pero no "
                                      "tiene lote." % (articulo.id),
                                      exc_info = True)
                    codlote = None
                if codlote != None and codlote not in lotes:
                ...
                try:
                    ...
                    self.logger.error("El artículo ID %d es fibra embolsada "
                                      "pero no tiene partida." % (articulo.id),
                                      exc_info = True)
                    codlote = None
                if codlote != None and codlote not in lotes:
                    lotes.append(codlote)
    ...
    def crear_servicio(self):
        # Datos a pedir: Concepto, descuento y precio... Bah, el descuento que
        # lo cambie en el TreeView.
    ...
    def drop_srv(self, boton):
        if self.wids['tv_servicios'].get_selection().count_selected_rows():
            selection = self.wids['tv_servicios'].get_selection()
    ...
    def add_transporte_a_cuenta(self, boton):
        ...
        if (self.objeto
            ...
            t = pclases.TransporteACuenta(concepto = "Transporte pagado.",
                    precio=0,
                    proveedor=None,
                    observaciones="Introduzca el precio y empresa "
                                  "transportista.",
                    fecha=mx.DateTime.localtime(),
                    albaranSalidaID=self.objeto.id)
            pclases.Auditoria.nuevo(t, self.usuario, __file__)
    ...
    def rellenar_comisiones(self):
        ...
        for comision in self.objeto.comisiones:
            ...
            try:
                fracompra = comision.serviciosTomados[0].facturaCompra
                numfactura = fracompra.numfactura
            except (IndexError, AttributeError):
                numfactura = ""
    ...
    def drop_comision(self, boton):
        ...
        """
        if self.wids['tv_comisiones'].get_selection().count_selected_rows():
            selection = self.wids['tv_comisiones'].get_selection()
...
# XXX Parte de facturación automática:
    def get_siguiente_numfactura(self, cliente):
        ...
        """
        # NOTA: Calcado de facturas_venta.py. Si cambio algo aquí, cambiar
        # allí y viceversa.
        numfactura = None
        if cliente.contador != None:
            cliente.contador.sync()
            numfactura = "%s%04d%s" % (cliente.contador.prefijo,
                                       cliente.contador.contador,
                                       cliente.contador.sufijo)
            # El número entre prefijo y sufijo pasa a 4 dígitos como mínimo
        return numfactura
            ...
            if ok:
                ...
                try:
                    ...
                    self.logger.error("%salbaranes_salida::generar_factura ->"
                            " Error al actualizar contador (probablemente "
                            "debido a concurrencia): %s" % (
                                self.usuario and self.usuario.usuario or "",
                                msg))
                    utils.dialogo_info(titulo = "ERROR",
    ...
    def init_structs_factura(self):
        fras = []
        ldvs_facturables = [ldv for ldv in self.objeto.lineasDeVenta
                            if not ldv.facturaVenta and not ldv.prefactura]
        srvs_facturables = [srv for srv in self.objeto.servicios
                            if not srv.facturaVenta and not srv.prefactura]
        ldvs = [ldv for ldv in self.objeto.lineasDeVenta]
        srvs = [srv for srv in self.objeto.servicios]
        ldvs_facturadas = [ldv for ldv in self.objeto.lineasDeVenta
                           if ldv.facturaVenta or ldv.prefactura]
        srvs_facturados = [srv for srv in self.objeto.servicios
                           if srv.facturaVenta or srv.prefactura]
        ...
        return (fras, ldvs_facturadas, srvs_facturados, ldvs_facturables,
                srvs_facturables)
    ...
    def crear_vencimientos_por_defecto(self, factura):
        ...
        if not vtos:
            if cliente.vencimientos != None and cliente.vencimientos != '':
                try:
                    ...
                    utils.dialogo_info(titulo = 'ERROR VENCIMIENTOS POR '
                                                'DEFECTO',
                            texto = 'Los vencimientos por defecto del cliente'
                                    ' no se pudieron procesar correctamente.\n'
                                    'Verifique que están bien escritos y el '
                                    'formato es correcto en la ventana de '
                                    'clientes.',
                            padre = self.wids['ventana'])
        ...
        if vtos:
            ...
            for incr in vtos:
                fechavto = (mx.DateTime.DateFrom(factura.fecha)
                    ...
                    for dia_estimado in diaest:
                        while True:
                            try:
                                ...
                                if dia_estimado <= 0:
                                    dia_estimado = 31
                        if fechaest < fechavto:     # El día estimado cae ANTES
                                    # del día del vencimiento. No es lógico,
                                    # la estimación debe ser posterior.
                            ...
                            try:
                                ...
                                # La ley de comercio dice que se pasa al
                                # último día del mes:
                                fechaest = mx.DateTime.DateTimeFrom(
                                        day = -1, 
                                        month = mes, 
    ...
    def descargar_de_terminal(self, boton):
        ...
        """
        self.logger.warning("%salbaranes_de_salida -> Iniciando "
                "descargar_de_terminal (salida de artículos automática)" % (
                    self.usuario and self.usuario.usuario + ": " or ""))
        ...
        while datos == None and not cancelar:
            ...
            if datos == None:
                cancelar = not utils.dialogo(titulo = "¿VOLVER A INTENTAR?",
                                             texto = "Se ha superado el tiempo"
                                                     " de espera.\n¿Desea "
                                                     "continuar?\n\n(Pulse "
                                                     "SÍ para volver a "
                                                     "intentar o NO para "
                ...
                for albaran in datos:
                    if albaran == self.objeto:
                        self.descargar_y_meter_articulos_en_albaran_actual(
                                datos[albaran])
                    else:
                        ...
                        utils.dialogo_info(titulo = "ALBARÁN INCORRECTO",
                                texto = "El albarán descargado (%s) no "
                                        "coincide con el albarán actual en "
                                        "ventana (%s).\nSe ignorará.\n\nNo "
                                        "borre la memoria del terminal y "
                                        "realice la descarga en el albarán "
                                        "correcto.\n\n\nSi ha leído más de "
                                        "un albarán en el terminal, a "
                                        "continuación se intentarán descargar"
                                        " también.",
                                padre = self.wids['ventana'])
    ...
    def descargar_y_meter_articulos_en_albaran_actual(self, datos):
        ...
        %s
        """ % ("\n".join([a.codigo for a in articulos_baja_calidad]))
        if articulos_baja_calidad == [] or utils.dialogo(
                titulo = "ARTÍCULOS DE BAJA CALIDAD",
                texto = texto,
                padre = self.wids['ventana']):
            self.asociar_articulos_a_albaran(articulos)
            self.objeto.calcular_comisiones()
...
def buscar_cliente(nombre, ventana_padre = None):
    ...
    clientes = pclases.Cliente.select(
                    pclases.Cliente.q.nombre.contains(nombre))
