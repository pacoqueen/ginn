...
class ConsumoBalasPartida(Ventana):
    """
    Clase de la ventana. Se abre al instanciarla.
    """
    ...
    def cambiar_peso_bala(self, cell, path, newtext):
        ...
        try:
            ...
            utils.dialogo_info(titulo='ERROR DE FORMATO',
                               texto='No introdujo un número válido',
                               padre=self.wids['ventana'])
            return
    ...
    def get_consumos_estimados(self, partida_carga, vpro):
        ...
        for partida in partida_carga.partidas:
            ...
            for rollo in rollos:
                vpro.set_valor(i/tot, 'Analizando geotextiles (%s)...' % (
                                                                rollo.codigo))
                try:
                    # Consumo estimado en base al peso REAL (sin embalaje y en
                    # kilos) de los rollos de la partida.
                    articulo = rollo.articulos[0]
                    cer = articulo.productoVenta.camposEspecificosRollo
                    peso_rollo = (rollo.peso - cer.pesoEmbalaje)
                    try:
                        merma = articulo.parteDeProduccion.merma
                    except AttributeError:
                        merma = 0
                    mermas.append(merma)
                    ...
                    self.logger.error("consumo_balas_partida.py "
                            "(get_consumos_estimados): ¡No se encontraron "
                            "artículos en el rollo ID %d!" % (rollo.id))
                i += 1
            for rollo in rollosd:
                ...
                try:
                    ...
                    self.logger.error("consumo_balas_partida.py "
                            "(get_consumos_estimados): "
                            "¡No se encontraron artículos en el "
                            "rollo_defectuoso ID %d!" % (rollo.id))
                i += 1
        try:
    ...
    def rellenar_balas(self):
        """
        Puebla el treeview de balas en la partida de carga.
        """
        model = self.wids['tv_balas'].get_model()
        model.clear()
        cantidad = 0
        ...
        if partida != None:
            ...
            consumos_estimados, merma = self.get_consumos_estimados(partida,
                                                                    vpro)
            self.wids['e_peso_gtx'].set_text("%s kg" % (
                                    utils.float2str(consumos_estimados, 2)))
            self.wids['label_gtx'].set_label("Peso aproximado geotextiles"
                    " producidos: \n"
                    "<small><i>Contando merma estimada media del %.2f%%."
                    "Sin embalaje.</i></small>" % (merma))
            # try:
            #     articulo = partida.rollos[0].articulos[0]
            #     producto = articulo.productoVenta.descripcion
            # except IndexError:
            #     producto = "SIN PRODUCCIÓN"
            # except:
            #     producto = "ERROR"
            ...
            for lote_id in productos:
                try:
                    lote = "%d (%s)" % (productos[lote_id]['lote'].numlote,
                                        productos[lote_id]['lote'].codigo)
                except AttributeError:
                    lote = ""
                try:
    ...
    def drop_bala(self, wid):
        """
        Quita una bala de la partida de carga y actualiza el treeview.
        """
        treeview = self.wids['tv_balas']
            ...
            for ide in resp:
                ...
                if bala.claseb:
                    if utils.dialogo(titulo='BALA MARCADA COMO BAJA CALIDAD',
                            texto='La bala está marcada como clase B. Esto '
    ...
    def set_partida(self, boton):
        """
        Hace activa una partida seleccionada para agregarle balas, imprimir su
        consumo, etc...
        """
        try:
            max_partida = pclases.PartidaCarga._queryOne(
                    "SELECT MAX(numpartida) FROM partida_carga")[0]
        ...
        try:
            try:
                ...
                partida = pclases.PartidaCarga.select(
                        pclases.PartidaCarga.q.numpartida == codigo)[0]
            except ValueError:
                partida = pclases.PartidaCarga.select(
                        pclases.PartidaCarga.q.codigo == codigo)[0]
            self.objeto = partida
            self.actualizar_ventana()
        except TypeError:
            return
        except IndexError:
            codigo = str(codigo)       # CHAPU
    ...
    def add_producto(self, boton):
        ...
        NOT IMPLEMENTED
        """
        # NO IMPLEMENTADO. Ni creo que llegue a hacer falta. (De momento se
        # queda como oculto en el glade).
        pass
    ...
    def descargar_de_terminal(self, boton):
        ...
        """
        self.logger.warning("%sconsumo_balas_partida::descargar_de_terminal "
                "-> Iniciando descarga de balas consumidas/a consumir "
                "automática." % (
                    self.usuario and self.usuario.usuario + ": " or ""))
        datos = None
        cancelar = False
            ...
            if datos == None:
                cancelar = not utils.dialogo(titulo="¿VOLVER A INTENTAR?",
                            texto="Se ha superado el tiempo de espera.\n"
                                  ...
                                  " a intentar o NO para cancelar la "
                                  "operación.)",
                            padre=self.wids['ventana'])
                ...
                for partida_carga in datos:
                    if partida_carga == self.objeto:
                        self.descargar_y_meter_balas_en_partida_carga_actual(
                                datos[partida_carga])
                    else:
                        self.descargar_y_meter_balas_en_partida_carga(
                ...
                utils.dialogo_info(titulo="ALBARÁN INTERNO CREADO",
                    texto="Albarán %s creado para la partida %s con %d balas."
                          "\n\nPulse «Aceptar» para continuar." % (
                              albaran.numalbaran, partida.codigo,
                              len(albaran.articulos)),
                    padre=self.wids['ventana'])
            else:
                self.logger.error("consumo_balas_partida::"
                        "descargar_y_meter_balas_en_partida_carga: "
                        "Se han importado balas a la partida ID %d y "
                        "sin embargo no se ha podido crear el albarán "
                        "interno." % (partida.id))
                # ventanapartida = ConsumoBalasPartida(usuario = self.usuario,
                #                                      objeto = partida)
                # from formularios import albaranes_de_salida
                # ventanaalbaran = albaranes_de_salida.AlbaranesDeSalida(
    ...
    def introducir_balas_en_partida(self, lista_balas, partida):
        ...
        if malas != []:
            ...
            utils.dialogo_info(titulo="BALAS INCORRECTAS",
                texto="Algunos códigos no se pudieron relacionar con la"
                      " partida %s:\n\n"  % (partida.codigo)
                      + texto_balas_malas
                      + "\n\nPulse «Aceptar» para continuar.",
                padre=self.wids['ventana'])
        ...
        if partida != None:
            ...
            if fecha:
                ...
                if albaranes_internos:
                    albaranes_internos.sort(
                            lambda a1, a2: (a1.fecha < a2.fecha and -1)
