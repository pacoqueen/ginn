...
from formularios.albaranes_de_salida import ajustar_existencias
from framework import pclases
from informes import geninformes
from formularios.prefacturas import bloquear_albaranes, descontar_existencias
from formularios.prefacturas import desglosar_ldvs_por_pedido
from formularios.prefacturas import buscar_cuentaOrigen
...
class FacturasVenta(Ventana):
    def __init__(self, objeto = None, usuario = None):
        ...
        if self.objeto and not isinstance(self.objeto, pclases.FacturaVenta):
            ...
            try:
                self.objeto = pclases.FacturaVenta.selectBy(
                                                numfactura = self.objeto)[0]
            except IndexError:
                try:
                    self.objeto = pclases.FacturaVenta.get(int(self.objeto))
                except (pclases.SQLObjectNotFound, ValueError, TypeError):
                    self.objeto = None
        connections = {'b_salir/clicked': self.salir,
                       ...
                       'b_vtos_defecto/clicked':
                            self.crear_vencimientos_por_defecto_y_actualizar,
    ...
    def es_diferente(self):
        ...
        condicion = condicion and (
                factura.observaciones
                    == self.wids['e_observaciones'].get_text())
        condicion = condicion and (
        ...
        try:
            ...
            condicion = condicion and (
                    utils.parse_porcentaje(
                        self.wids['e_irpf'].get_text(), fraccion = True)
                    == factura.irpf)
        except ValueError, msg:
            self.logger.error("facturas_venta.py::es_diferente ->"
                    " Error, probablemente al parsear pordentaje: %s" % (msg))
    ...
    def inicializar_ventana(self):
        ...
        cols = (('Código', 'gobject.TYPE_STRING', True, True, False, None),
                ...
                ('Total de línea', 'gobject.TYPE_STRING',
                                                    False, True, False, None),
        ...
        cols = (('Vencimiento', 'gobject.TYPE_STRING', True, True, True,
                ...
                ('Cobrado o documentado', 'gobject.TYPE_BOOLEAN',
                    True, True, False, self.cobrar_o_anular_vto),
        ...
        try:
            ...
            self.logger.error("facturas_venta::inicializar_ventana ->"
    ...
    def add_ticket(self, boton):
        ...
        if a_buscar != None:
            ...
            if resultados.count() > 1:
                listatickets = self.refinar_resultados_busqueda_ticket(
                                                                    resultados)
                if listatickets:
        ...
        for srv in self.objeto.servicios:
            if (srv.pedidoVenta == None
                ...
                if srv.albaranSalida != None:
                    if (not srv.albaranSalida.numalbaran
                            in peds[srv.pedidoVenta.numpedido]):
                        peds[srv.pedidoVenta.numpedido].append(
    ...
    def insertar_servicio_en_model(self, srv, padre, model):
        ...
        model.append(padre,
                     ("TRANSPORTE Y OTROS",
                      ...
                      utils.float2str(totlinea, 3, autodec = True),
                      '-',
                      srv.albaranSalida and srv.albaranSalida.numalbaran
                                                                        or '-',
                      srv.id))
    ...
    def rellenar_vencimientos(self):
        ...
        """
        # PLAN: Colorear vencimientos incumplidos, con estimaciones
        #       rebasadas...
        # OJO: Además, también hay que emparejar por proximidad de fechas
        #      los pagos realizados (clase Pago) con los vencimientos a los
        #      que corresponderían (no hay relación directa).
        vtos = self.preparar_vencimientos()
        # FIXME: (?) Se llama a preparar_vencimientos hasta 4 veces (!) al
        ...
        if (total_vtos != 0 and
            ...
            if utils.dialogo(titulo = "VENCIMIENTOS NO COINCIDEN",
                ...
                self.rellenar_vencimientos()
                if vtos_creados == SIN_DATOS_PARA_VENCIMIENTOS_POR_DEFECTO:
                    utils.dialogo_info(titulo="SIN DATOS",
                        texto="El cliente no tiene datos suficientes para "
    ...
    def add_pedido(self, idpedido):
        ...
        for ldv in [l for l in pedido.lineasDeVenta
                    if l.facturaVenta == None and l.prefactura == None]:
    ...
    def add_albaran(self, idalbaran):
        ...
        for ldv in [l for l in albaran.lineasDeVenta
                    if l.facturaVenta == None and l.prefactura == None]:
            ldv.facturaVenta = factura
        for srv in [s for s in albaran.servicios
                    if s.facturaVenta == None and s.prefactura == None]:
    ...
    def add_ldv(self, idldv):
        ...
        if ldv.facturaVenta == None and ldv.prefactura == None:
            ...
            tolerancia = 0.1
            if pclases.DEBUG:
                myprint("DEBUG: (TEMPORAL, NO ES UN ERROR): ¿%f == %f - %f?"
            ...
            if len(factura.lineasDeVenta) == 0:
                factura.descuento = (ldv.pedidoVenta
                                     and ldv.pedidoVenta.descuento or 0)
            ldv.facturaVenta = factura
        else:
    ...
    def buscar_articulos(self):
        a_buscar = utils.dialogo_entrada('Introduzca código de producto, '
                                         'código interno o descripción',
                                         'BUSCAR ARTÍCULO',
                                         padre = self.wids['ventana'])
        # Tengo que buscar entre balas, rollos y productos para obtener una
        # lista de artículos.
        ...
        rollos = pclases.Rollo.select(
                    pclases.Rollo.q.codigo.contains(a_buscar))
    ...
    def crear_nueva_factura(self, widget):
        ...
        (ultima_factura,
         factura_anterior,
         factura_posterior,
         ok) = self.chequear_restricciones_nueva_factura(cliente, numfactura,
                                                         fecha)
        if not ok:
            txt = """
        ...
        try:
            ...
            self.logger.error("facturas_venta::crear_nueva_factura -> "
                              "No se encontraron los datos de la empresa. "
                              "Excepción: %s" % (msg))
            irpf = 0.0
        factura = pclases.FacturaVenta(fecha = fecha,
                                       ...
                                       bloqueada = False,
                                       irpf = irpf)
                # ¡¡¡¿Por qué sigue teniendo el cliente el IVA como entero en
                # vez de fracción de 1?!!!
        ...
        if numero_contador > cliente.contador.contador:
            # Si crea una factura que no sea la última (para rellenar algún
            # hueco o algo), NO DEBE ACTUALIZAR EL CONTADOR.
            cliente.contador.contador = numero_contador
        # DONE: OJO: NOTA: DANGER: WARNING: PELIGRO: AAARRRGGGHHHH: ¿Qué
        # pasará cuando cambie de año y cambien en Geotexan de sufijo en el
        # contador? ¿Debe comenzar la numeración por 0 de nuevo? Mirar y
        # preguntar antes de que llegue el cierre de año y me pille en bragas,
        # aunque para mí que se dijo que no, que continuaba por el último
        # número del año anterior pero cambiando el sufijo.
        # Lo que se hace es crear un contador nuevo con el prefijo que
        # corresponda (p. ej: F70xxxxx) y asignar a todos los clientes del
        # antiguo contador al nuevo contador desde la ventana contadores.
    ...
    def chequear_restricciones_nueva_factura(self, cliente, numfactura, fecha):
        """
        Devuelve la última factura de la serie del cliente recibido y un
        booleano que valdrá True si pasa las restricciones o False si no las
        cumple.
    ...
    def guardar(self, widget):
        ...
        """
        factura = self.objeto
        # Campos del objeto que hay que guardar: IVA. Fecha y numfactura no,
        # tienen botón propio.
        # Desactivo el notificador momentáneamente
        factura.notificador.set_func(lambda: None)
        # Actualizo los datos del objeto
        try:
            iva = utils.parse_porcentaje(self.wids['e_iva'].get_text(),
                                         fraccion = True)
        except ValueError:
            iva = 0
        factura.iva = iva
        try:
            irpf = utils.parse_porcentaje(self.wids['e_irpf'].get_text(),
                                          fraccion = True)
            if irpf > 0.0:      # El IRPF _siempre_ se descuenta (lo retiene
                                # el cliente).
                irpf *= -1
        except ValueError:
            irpf = 0
        ...
        if factura.clienteID and factura.cliente.es_extranjero() and (
                factura.cliente.get_iva_norm() != 0 or factura.iva != 0):
    ...
    def cambiar_fecha(self, w):
        ...
        factura_anterior, factura_posterior, ok = self.chequear_cambio_fecha(
                                                    self.objeto, nueva_fecha)
        if ok:
            fechanterior = self.objeto.fecha
            ...
            utils.dialogo_info(titulo = "ERROR EN FECHA",
                               texto = """
            No puede asignar una fecha anterior a la de la factura"""
            """ inmediatamente
    ...
    def chequear_cambio_fecha(self, factura, fecha):
        ...
        # Any better?
        # FIXME: Si la fra. es del año pasado (prefijo es EX13 en lugar de EX14
        #        por ejemplo) o el cliente ha cambiado de contador, cacotas.
        ...
        if pclases.DEBUG:
            myprint([f.numfactura for f in facturas])
        facturas.sort(lambda f1, f2: f1.get_numero_numfactura()
                                     - f2.get_numero_numfactura())
        ...
        if nuevo_numero == None:
            ...
            if ok:
                ...
                utils.dialogo_info(titulo = "ERROR EN NÚMERO DE FACTURA",
                                   texto = """
                No puede asignar un número inferior al de la factura"""
                """ inmediatamente
                anterior según su fecha, ni posterior al de la siguiente"""
                """ factura.
                Factura anterior:
                    - Nº factura %s.
                    - Fecha %s.
                ...
                """ % (factura_anterior and factura_anterior.numfactura or "-",
                       factura_anterior
                            and utils.str_fecha(factura_anterior.fecha) or "-",
                       factura_posterior
                            and factura_posterior.numfactura or "-",
                       factura_posterior
                            and utils.str_fecha(factura_posterior.fecha)
                            or "-"))
    ...
    def chequear_cambio_numero(self, factura, numero):
        ...
        facturas = pclases.FacturaVenta.select("cliente_id IN (%s)" % clientes)
        # Any better?
        facturas = [f for f in facturas if f != factura]    # Facturas del
                                        # mismo contador quitando la actual.
        micontador = factura.get_contador()
        facturas = [f for f in facturas if f.get_contador() == micontador]
        ...
        i_actual = 0
        while (i_actual < len(facturas)
            and facturas[i_actual].get_numero_numfactura_contador() < numero):
            i_actual += 1
        anteriores = facturas[:i_actual]
        posteriores = facturas[i_actual:]
        if pclases.DEBUG:
            myprint("--------------------------------------")
            myprint("facturas_venta::chequear_cambio_numero")
            myprint([f.numfactura for f in facturas])
            myprint([f.numfactura for f in anteriores])
            myprint(factura.numfactura, " -> ", numero)
            myprint([f.numfactura for f in posteriores])
            myprint("--------------------------------------")
        factura_anterior = anteriores and anteriores[-1] or None
        factura_posterior = posteriores and posteriores[0] or None
        ok = True
        if factura_anterior:
            if pclases.DEBUG:
                myprint(factura_anterior.get_numero_numfactura(), numero,
                        factura.fecha)
            ok = ok and factura_anterior.fecha <= factura.fecha
        if factura_posterior:
            if pclases.DEBUG:
                myprint(factura_posterior.get_numero_numfactura(), numero,
                        factura.fecha)
            ok = ok and factura_posterior.get_numero_numfactura() > numero
            # TODO: ¿Y no chequeo la fecha, solo el número?
        else:   # No hay facturas posteriores, por tanto, actualizo el contador
                # a la última factura (la actual)
    ...
    def get_numero_numfactura(self, numfactura):
        ...
        numero = numfactura.replace(
                            contador.prefijo, '').replace(contador.sufijo, '')
        try:
            return int(numero)
        except:
            return None
    ...
    def borrar_factura(self, boton):
        ...
        for ldv in factura.lineasDeVenta:
            ...
            # De paso, intento eliminarla por completo, por si no tiene
            # relación con pedidos y demás.
            ldv.eliminar()
        ...
        try:
            ...
            # DONE: ¿Al borrar la última factura del contador no lo
            # decrementa? ¿Por qué se cambió, si eso estaba hecho?
            numero = numfactura.replace(contador.prefijo, "")
            numero = numero.replace(contador.sufijo, "")
            try:
                numero = int(numero)
            except ValueError, msg:
                self.logger.error("factura_venta.py::borrar_factura ->"
                        " no se pudo determinar el número (numérico) de "
                        "factura. Excepción: %s" % msg)
            else:
                if numero == contador.contador - 1:  # Era la última de su
                                            # serie. Decremento el contador.
                    contador.contador -= 1
                    contador.sync()
    ...
    def drop_ldv(self, boton):
        ...
        if ldv.albaranSalidaID == None:
            ...
            try:
                ...
                myprint("facturas_venta.py (drop_ldv): LDV ID %d sin factura"
                        " ni albarán, pero no se pudo eliminar." % (ldv.id))
        self.rellenar_contenido()
    ...
    def pedir_producto(self):
        ...
        """
        productos = []
        txt = utils.dialogo_entrada(
                'Introduzca código, nombre o descripción de producto.\n'
                'Puede usar también el código de COMPOSAN si está buscando'
                ' geotextiles.',
                'CÓDIGO PRODUCTO',
                padre = self.wids['ventana'])
        if txt != None:
    ...
    def cambiar_cantidad_srv(self, cell, path, texto):
        ...
        try:
            ...
            self.rellenar_totales()
            self.rellenar_vencimientos()    # Para que verifique si los totales
                                            # coinciden
        except:
    ...
    def cambiar_precio_srv(self, cell, path, texto):
        ...
        try:
            ...
            if pclases.DEBUG:
                myprint(srv.precio, utils._float(texto), texto)
            self.rellenar_servicios()
            self.rellenar_vencimientos()    # Para que verifique si los totales
                                            # coinciden
        except:
    ...
    def rellenar_servicios(self):
        ...
        for servicio in self.objeto.servicios:
            if pclases.DEBUG:
                myprint(servicio.precio, utils._float(servicio.precio))
    ...
    def clon_srv(self, boton):
        ...
        res = utils.dialogo_resultado(filas,
                "SELECCIONE SERVICIO",
                cabeceras = ('ID', 'Concepto', 'Precio', 'Facturado en',
                             'Cliente'),
                multi = True,
                padre = self.wids['ventana'])
        if res[0] > 0:
    ...
    def drop_vto(self, boton):
        ...
        """
        selection = self.wids['tv_vencimientos'].get_selection()
        model, itr = selection.get_selected()
        if itr == None: return
        ids = model[itr][-1]
    ...
    def cambiar_vto(self, cell, path, texto):
        ...
        self.rellenar_vencimientos()        # Para no sobrecargar mucho la red
                                            # volviendo a rellenar LDVs y tal.
    ...
    def cambiar_estimado(self, cell, path, texto):
        ...
        try:
            ...
            return
        idvto = int(
            self.wids['tv_vencimientos'].get_model()[path][-1].split(',')[1])
            # Al escribirlo no parecía tan lioso. Lo juro.
        if idvto > 0:    # Es -1 si no había.
    ...
    def cambiar_cobro(self, cell, path, texto):
        ...
        """
        idcobro = int(
            self.wids['tv_vencimientos'].get_model()[path][-1].split(',')[2])
            # Al escribirlo no parecía tan lioso. Lo juro.
        if texto == "":
    ...
    def cambiar_cantidad(self, cell, path, texto):
        ...
        if idvto > 0:    # Es -1 si no había.
            vto = pclases.VencimientoCobro.get(idvto)
            vto.importe = cantidad
        elif idvto == -1:   # Para el resto de valores rebota-rebota y en
                            # tu culo explota.
            factura = self.objeto
    ...
    def cambiar_precio(self, cell, path, texto):
        ...
        idldv = model[path][-1]
        ldv = pclases.LineaDeVenta.get(idldv)
        if not ldv.pedidoVentaID or (ldv.pedidoVentaID
                                     and not ldv.pedidoVenta.bloqueado):
            ...
            if self.usuario and self.usuario.nivel >= 2:
                utils.dialogo_info(titulo = "OPERACIÓN NO PERMITIDA",
                    texto = "El pedido de venta (%s) se encuentra bloqueado.\n"
                            "Cambie el precio allí o desbloquéelo primero." % (
                ...
                if utils.dialogo(titulo = "PEDIDO BLOQUEADO",
                    ...
                    pclases.Auditoria.modificado(ldv, self.usuario, __file__,
                        "Precio actualizado de %f a %f." % (precio_anterior,
                                                            ldv.precio))
                    self.rellenar_ldvs()
                    self.rellenar_vencimientos()        # Redundante. Esto se
                                    # puede refactorizar. ¿Es hora de probar
                                    # el Bicycle Repair Man?
    ...
    def cambiar_descuento(self, cell, path, texto):
        ...
        idldv = model[path][-1]
        ldv = pclases.LineaDeVenta.get(idldv)
        if ((ldv.pedidoVenta and not ldv.pedidoVenta.bloqueado)
                or not self.usuario):
            ...
            try:
                ...
                if not condicion_modificacion and self.usuario != None:
                    ...
                    ldv.descuento = cantidad
                    self.rellenar_ldvs()
                    self.rellenar_vencimientos()    # Para que verifique si
    ...
    def imprimir(self, boton):
        ...
        # desglosar_albaranes = (
        #   len(factura.get_albaranes(incluir_nones = True)) > 1 and
        ...
        if len(lineas) > 0:
            ...
            if not self.objeto.bloqueada:
                txt = """
                    ...
                    pendiente de verificar hasta que sea imprimida """\
                    """correctamente.
    ...
    def buscar_abonos(self, w):
        ...
        # Si tienen pagos de abono en la factura de abono es que ya se ha
        # usado en otra factura.
    ...
    def crear_vencimientos_por_defecto_y_actualizar(self, boton):
        try:
            vtos_creados = self.objeto.crear_vencimientos_por_defecto()
    ...
    if not usuario or not smtppass:
        smtppass = utils.dialogo_entrada(
                    ...
                    texto = "Introduzca contraseña del servidor de salida:\n"
                            "(Puede dejarla en blanco si el servidor no "
    ...
    if cliente and cliente.enviarCorreoFactura:
        ...
        if remitente and server:
            ...
            correos = utils.dialogo_entrada(
                ...
                texto = "Confirme o introduzca la dirección de correo "
                        "electrónico del cliente.\nSi no desea recibir copia,"
                        " borre su propia dirección:",
                padre = padre,
            ...
            if correos:
                ...
                try:
                    ...
                    textoerror = "%sfacturas_venta.py::"\
                                 "enviar_factura_por_correo -> "\
                                 "Error al enviar el factura ID %d. Mensaje "\
                                 "de la excepción: %s" % (
                                    usuario and usuario.usuario + ": " or "",
                                    factura and factura.id or 0,
                                    msg)
                    if logger:
...
def generar_recibo(factura, usuario=None, logger=None, ventana_padre=None):
    ...
    for vto in vtos:
        ...
        try:
            ...
            observaciones = "Recibo bancario número %d con fecha de emisión"\
                            " %s." % (recibo.numrecibo,
                                      utils.str_fecha(recibo.fechaLibramiento))
            ...
            utils.dialogo_info(titulo = "ERROR",
                texto = "Ocurrió un error al crear el nuevo recibo.\n"
                        ...
                        "fuera necesario.",
                padre = ventana_padre)
            txt = "%sfacturas_venta::nuevo -> Error al crear nuevo recibo."\
                  " Mensaje de la excepción: %s" % (
                          usuario and usuario.usuario or "", msg)
            if logger:
                logger.error(txt)
            if pclases.DEBUG:
                myprint(txt)
