...
###################################################################
## resultados_encogimiento.py - Resultados de pruebas de elongación. 
###################################################################
## NOTAS:
##  
###################################################################
...
class ResultadosEncogimiento(Ventana):
    def __init__(self, objeto = None, usuario = None):
        ...
        """
        Ventana.__init__(self, 'resultados_encogimiento.glade', objeto, usuario = usuario)
    ...
    def rellenar_pruebas(self):
        ...
        pruebas = pclases.PruebaEncogimiento.select(pclases.PruebaEncogimiento.q.loteID == self.lote.id)
    ...
    def add(self, w):
        if self.lote != None:
            ...
            if resultado == '':
                ...
                return
            try:
                prueba = pclases.PruebaEncogimiento(fecha = time.strptime(fecha, '%d/%m/%Y'),
                                                    resultado = resultado,
                                                    lote = self.lote)
    ...
    def drop(self, w):
        ...
        if itr != None and utils.dialogo(titulo = 'BORRAR PRUEBA', texto = '¿Está seguro?'):
            ide = model[itr][-1]
            prueba = pclases.PruebaEncogimiento.get(ide)
    ...
    def cambiar_fecha(self, cell, path, texto):
        model = self.wids['tv_pruebas'].get_model()
        prueba = pclases.PruebaEncogimiento.get(model[path][-1])
        try:
