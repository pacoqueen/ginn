...
def build_encabezado(datos_empresa = []):
    ...
    estilo_encabezado.rightIndent = PAGE_WIDTH * 0.4
    estilo_encabezado.alignment = enums.TA_CENTER
    estilo_encabezado.spaceAfter = estilo_encabezado.spaceBefore = 6
    for linea in datos_empresa:
        p = Paragraph(escribe(linea), estilo_encabezado) 
        cabecera.append(p)
        estilo_encabezado.fontSize -= 1
        if estilo_encabezado.spaceAfter > 0:
