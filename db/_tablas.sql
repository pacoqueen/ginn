...
CREATE TABLE proveedor(
    ...
    motivo TEXT DEFAULT '',     -- NEW! 05/12/06. Si está inhabilitado no se
                                ...
                                -- compra.(CWT)
    iva FLOAT DEFAULT 0.16,     -- NEW! 30/01/07. Iva por defecto del
...
CREATE TABLE categoria_laboral(
    ...
    precio_hora_regular FLOAT DEFAULT 0.0 -- NEW! 16/09/2008. Necesario para                                              -- calcular costes de líneas.
);
...
-- existencias de ese producto en esa fecha concreta.
...
CREATE TABLE bolsa(
    id SERIAL PRIMARY KEY,
    caja_id INT REFERENCES caja, 
    numbolsa INT8 UNIQUE,
    codigo TEXT,    -- Empezarán por K
    fechahora TIMESTAMP DEFAULT LOCALTIMESTAMP(0), 
    peso FLOAT, -- Aunque siempre será el mismo que el del producto, pero 
                -- para optimizar cálculos lo anoto en el registro. PESO 
                -- EN KILOGRAMOS. 
    claseB BOOLEAN DEFAULT FALSE,   -- Una bolsa puede ser B por tres motivos:
        -- 1.- Que las cajas lleven menos de 40 bolsas.
        -- 2.- Que la fibra de origen sea B.
        -- 3.- Que lo marque expresamente el usuario así en el parte (motivo 
        --     por el que está aquí y no es campo calculado).
    observaciones TEXT DEFAULT ''
);
...
CREATE TABLE articulo(
    ...
    almacen_id INT REFERENCES almacen DEFAULT NULL,  
    bolsa_id INT REFERENCES bolsa DEFAULT NULL,     -- NEW! 18/05/2009
    CHECK (NOT(bala_id <> NULL
               AND rollo_id <> NULL
               AND bigbag_id <> NULL
               AND rollo_defectuoso_id <> NULL
               AND bala_cable_id <> NULL
               AND rollo_c_id <> NULL
               AND bolsa_id <> NULL))
);
...
CREATE TABLE obra(
    ...
    observaciones TEXT DEFAULT '', 
    generica BOOLEAN DEFAULT FALSE  -- NEW! 27/07/2007: CWT: Si 
...
CREATE TABLE factura_venta(
    ...
    observaciones TEXT DEFAULT '',
    iva FLOAT DEFAULT 0.16,    -- Si el cliente no tiene un IVA válido
                               -- definido, se usará el 16%.
...
-- Líneas de venta --
---------------------
-- TODO: Empiezo a echar en falta un campo de observaciones aquí para
...
CREATE TABLE linea_de_compra(
    ...
    carga_silo_id INT REFERENCES carga_silo DEFAULT NULL,   -- NEW! 01/11/06
        ...
        -- copias de seguridad)
    iva FLOAT DEFAULT 0.16      -- NEW! 15/02/07. IVA de la LDC. Por
...
CREATE TABLE servicio_tomado(
    ...
    iva FLOAT DEFAULT 0.16      -- NEW! 15/02/07. IVA del servicio para
...
CREATE TABLE datos_de_la_empresa(
    ...
    iva FLOAT DEFAULT 0.16, -- IVA soportado por defecto, sin contar R.E.
...
CREATE INDEX bsid ON articulo (bolsa_id);
...
CREATE UNIQUE INDEX idbolsa ON bolsa (id);      -- NEW! 20/07/2009
CREATE INDEX bolsaid ON articulo (bolsa_id);    -- NEW! 20/07/2009
CREATE UNIQUE INDEX idcaja ON caja (id);        -- NEW! 20/07/2009
CREATE INDEX cajaid ON bolsa (caja_id);         -- NEW! 20/07/2009
